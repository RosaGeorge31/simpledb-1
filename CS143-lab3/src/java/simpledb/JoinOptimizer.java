    	...
    	for (int i = 1; i <= joins.size(); i++) {
    		for (Set<LogicalJoinNode> joinSet : enumerateSubsets(joins, i)) {
    			...
    			for (LogicalJoinNode joinPlan : joinSet) {
    				CostCard cost = computeCostAndCardOfSubplan(stats, filterSelectivities, joinPlan, joinSet, bestcost, planCache);
    				if (cost != null) {
    					...
    					planCache.addPlan(joinSet, bestcost, bestcard, bestplan);
    				}
    			}
    		}
    	}
